...
public class MessageServiceImpl implements MessageService {
    ...
    private MessageSummaryDTO mapToSummaryDTO(Message message) {
        // Récupérer le contact via Echange pour le chip "Voir contact"
        Long contactId = null;
        String contactEmail = null;
        String contactNom = null;
        try {
            Contact contact = message.getEchange().getContact();
            if (contact != null) {
                contactId    = contact.getId();
                contactEmail = contact.getEmail();
                contactNom   = contact.getNom();
            }
        } catch (Exception ignored) { /* Echange ou Contact peut être null */ }
    ...
    private MessageDetailDTO mapToDetailDTO(Message message) {
        ...
        if (message.getAgent() != null) {
            ...
            agentDTO = AgentSummaryDTO.builder()
                    .id(a.getId()).model(a.getModel())
                    .actif(a.getActif()).dateActivation(a.getDateActivation())
                    .build();
        }
        ...
        if (message.getAnalyse() != null) {
            ...
            analyseDTO = AnalyseSummaryDTO.builder()
                    .id(an.getId()).description(an.getDescription())
