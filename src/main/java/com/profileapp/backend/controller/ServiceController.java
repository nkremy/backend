...
/**
 * CONTROLLER — Gestion admin des services (pôles)
...
public class ServiceController {
    ...
    public ResponseEntity<ApiResponse<ServiceDetailDTO>> getServiceById(@PathVariable Long id) {
        return ResponseEntity.ok(ApiResponse.success(
                ...
                serviceCatalogueService.getServiceById(id)));
    }
    ...
    public ResponseEntity<ApiResponse<List<ServiceSummaryDTO>>> rechercherServices(
            @RequestParam String motCle) {
        return ResponseEntity.ok(ApiResponse.success(
                "Recherche effectuée avec succès",
                serviceCatalogueService.rechercherParMotCle(motCle)));
    }
}
